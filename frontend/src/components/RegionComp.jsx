...
const RegionComp = () => {
  ...
  const translations = {
    EN: {
      ...
      employmentBusiness: 'Employment Level in Business Sector (thousand person)',
    ...
    GE: {
      pageTitle: 'ძირითადი მაჩვენებლების შედარება საქართველოს რეგიონების მიხედვით',
      ...
      employmentBusiness: 'დასაქმებულთა რაოდენობა - ბიზნეს სექტორში (ათასი კაცი)',
      averageSalary: 'დასაქმებულთა საშუალოთვიური ხელფასი - ბიზნეს სექტორში (ლარი)',
      registeredEntities: 'რეგისტრირებული ეკონომიკური სუბიექტების რაოდენობა (ერთეული)',
      activeEntities: 'მოქმედი ეკონომიკური სუბიექტების რაოდენობა (ერთეული)',
      newlyRegistered: 'ახლადრეგისტრირებული ეკონომიკური სუბიექტების რაოდენობა (ერთეული)'
    }
  };
    ...
    const fetchRegions = async () => {
      try {
        ...
        const fallbackRegions = [
          ...
          { value: 6, label: language === 'EN' ? 'Samtskhe-Javakheti' : 'სამცხე-ჯავახეთი' },
          { value: 7, label: language === 'EN' ? 'Kvemo Kartli' : 'ქვემო ქართლი' },
          { value: 8, label: language === 'EN' ? 'Shida Kartli' : 'შიდა ქართლი' },
          { value: 9, label: language === 'EN' ? 'Mtskheta-Mtianeti' : 'მცხეთა-მთიანეთი' },
          { value: 10, label: language === 'EN' ? 'Racha-Lechkhumi and Kvemo Svaneti' : 'რაჭა-ლეჩხუმი და ქვემო სვანეთი' },
          { value: 11, label: language === 'EN' ? 'Samegrelo-Zemo Svaneti' : 'სამეგრელო-ზემო სვანეთი' }
        ];
  ...
  const handleSearch = async () => {
    if (selectedRegions.length === 0 || selectedIndicators.length === 0) {
      alert(language === 'EN' ? 'Please select both regions and indicators' : 'გთხოვთ აირჩიოთ რეგიონები და მაჩვენებლები');
      return;
    }
    ...
    try {
      ...
      alert(language === 'EN' ? 'Error creating comparison table' : 'შედარების ცხრილის შექმნისას მოხდა შეცდომა');
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatComparisonData = (allRegionsData) => {
    ...
    selectedIndicators.forEach(indicator => {
      ...
      selectedRegions.forEach(region => {
        const regionData = allRegionsData.find(item => item.value === region.value);
        if (regionData) {
          // Map indicator to field name and get value
          const fieldValue = getFieldValueByIndicator(regionData, indicator.key);
  ...
  const handleExport = () => {
    if (!showTable || comparisonData.length === 0) {
      alert(language === 'EN' ? 'No data to export' : 'გასატანი მონაცემები არ არის');
      return;
    }
              ...
              <table className="w-full table-auto">
                <tbody>
                  {comparisonData.map((row, rowIndex) => (
                    <tr key={rowIndex} className={rowIndex === 0 ? '' : 'hover:bg-gray-50/50 transition-colors duration-200'}>
