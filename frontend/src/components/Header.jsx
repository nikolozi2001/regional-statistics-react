...
const HEADER_CLASSES =
  "w-full bg-white border-b-2 border-gray-300 shadow-lg py-4 sticky top-0 z-[1000]";
const CONTAINER_CLASSES =
  "w-full mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between min-h-[80px] flex-col md:flex-row gap-4 md:gap-0";
const LOGO_CONTAINER_CLASSES =
  "flex-shrink-0 flex items-center order-2 md:order-1";
const LOGO_CLASSES =
  "h-12 sm:h-14 md:h-15 w-auto object-contain transition-all duration-300 ease-in-out animate-fade-in";
const TITLE_CONTAINER_CLASSES =
  "flex-1 flex justify-center items-center px-0 md:px-8 order-1 md:order-2";
const RIGHT_SECTION_CLASSES =
  "flex-shrink-0 flex flex-col items-end gap-3 order-3 md:order-3";
const LANGUAGE_CONTAINER_CLASSES = "flex items-center";
const COMPARISON_BUTTONS_CLASSES =
  "flex flex-col sm:flex-row items-center gap-2";
const LANGUAGE_SWITCHER_CLASSES =
  "flex items-center rounded-3xl p-1 backdrop-blur-sm border border-white/20 shadow-sm animate-scale-in";
const DIVIDER_CLASSES = "text-gray-400 font-light mx-2 select-none";
const FLAG_CLASSES =
  "w-6 h-4 object-cover rounded-sm transition-all duration-200 hover:scale-110 group-hover:brightness-110";
...
const Header = memo(
  ({
    ...
    // Check if we should hide the Gender Statistics button
    const shouldHideGenderStats =
      location.pathname === "/" ||
    ...
    const comparisonButtons = useMemo(() => {
      return {
        regions:
          language === "EN" ? "Comparison of Regions" : "რეგიონების შედარება",
        municipalities:
          language === "EN"
          ...
          <div className={RIGHT_SECTION_CLASSES}>
            ...
            <div className={LANGUAGE_CONTAINER_CLASSES}>
              ...
              >
                ...
                {/* Divider */}
                <span className={DIVIDER_CLASSES} aria-hidden="true">
                  |
                </span>
