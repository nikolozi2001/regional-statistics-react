      ...
      .filter(
        (item) =>
          ...
          (item.value.includes("ჯანმრთელობის") ||
            ...
            item.value.includes(
              "Number New Cases of Malignant Neoplasm by Sex"
            ) ||
            item.value.includes(
              "New cases of the diseases of the circulatory system by sex"
            ) ||
            item.value.includes(
              "New cases of the diseases of the respiratory system by sex"
            ) ||
            item.value.includes(
              "Morbidity of patients with syphilis and gonorrheae by Sex"
            ) ||
            item.value.includes("Number of HIV infection/AIDS cases by sex") ||
            item.value.includes(
              "Morbidity of patients with tuberculosis by sex"
            ) ||
      ...
      .filter(
        (item) =>
          ...
          (item.value.includes("სოციალური უზრუნველყოფის") ||
            ...
            item.value.includes(
              "Number of Internally Displaced Disability Persons Receiving Social Package"
            ) ||
            item.value.includes(
              "The number of persons with disabilities registered in the Unified database of targeted social assistance program"
            ) ||
            item.value.includes(
              "Number of disabilitie persons, registered in the Unified database of targeted social assistance program, receiving subsistence a"
            ) ||
            item.value.includes(
              "Number of Disability Persons Receiving Social Package"
            ) ||
            item.value.includes(
              "The number of newly registered beneficiaries of the state grant(s) based on the status of a person with disabilities by sex"
            ))
      );
  };
            ...
            <div key={key} className="group">
              ...
              >
                <Collapsible.Trigger className="w-full text-left py-3 px-4 rounded-xl hover:bg-gray-50/80 transition-all duration-300 ease-out flex items-center justify-between group-hover:shadow-sm border border-transparent hover:border-gray-100">
                  ...
                  {children.length > 0 ? (
                    <ChevronDownIcon
                      className={`w-4 h-4 text-gray-400 group-hover:text-gray-600 transform transition-all duration-300 ease-out ${
                        isOpen ? "rotate-180" : ""
                      }`}
                    />
                  ) : (
                    <FaFileExcel className="w-4 h-4 text-green-500 group-hover:text-green-600 transition-colors duration-200" />
                  )}
                </Collapsible.Trigger>
                    ...
                    <div className="px-4 pb-2 space-y-1">
                      {children.map((child, index) => (
                        ...
                        >
                          ...
                          <span className="block truncate group-hover/item:font-medium transition-all duration-200">
                            {child.value}
                          </span>
                        </button>
                      ))}
              ...
              {key === "population" && demographicCategoriesData.length > 0 && (
                <div className="mt-2 group">
                  <Collapsible.Root
                    ...
                    onOpenChange={(open) =>
                      setOpenSections((prev) => ({ ...prev, demography: open }))
                    }
                  >
                    <Collapsible.Trigger className="w-full text-left py-3 px-4 rounded-xl hover:bg-gray-50/80 transition-all duration-300 ease-out flex items-center justify-between group-hover:shadow-sm border border-transparent hover:border-gray-100">
                      <span className="text-sm font-bold text-gray-700 group-hover:text-gray-900 transition-colors duration-200">
                      ...
                      {demographicCategoriesData.length > 0 ? (
                        <ChevronDownIcon
                          className={`w-4 h-4 text-gray-400 group-hover:text-gray-600 transform transition-all duration-300 ease-out ${
                            openSections.demography ? "rotate-180" : ""
                          }`}
                        />
                      ) : (
                        <FaFileExcel className="w-4 h-4 text-green-500 group-hover:text-green-600 transition-colors duration-200" />
                    ...
                    <Collapsible.Content className="overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-top-1 data-[state=open]:slide-in-from-top-1">
                      <div className="px-4 pb-2 space-y-1">
                        {demographicCategoriesData.map(
                          ({ key: demKey, value: demValue }) => {
                            const children = getChildrenForCategory(demKey);
                            const isDemographicSubOpen =
                              openSections[`demography-${demKey}`] || false;
                              ...
                              >
                                <div className="ml-2 border-l border-gray-100 pl-4 group/sub">
                                  <Collapsible.Trigger className="w-full text-left py-2 px-3 rounded-lg hover:bg-gray-50/50 transition-all duration-200 ease-out flex items-center justify-between">
                                    <span className="text-xs font-medium text-gray-600 group-hover/sub:text-gray-800 transition-colors duration-200">
                                      {demValue}
                                    </span>
                                    {children.length > 0 ? (
                                      <ChevronDownIcon
                                        className={`w-3 h-3 text-gray-400 group-hover/sub:text-gray-600 transform transition-all duration-200 ease-out ${
                                          isDemographicSubOpen
                                            ? "rotate-180"
                                            : ""
                                        }`}
                                      />
                                    ) : (
                                      <FaFileExcel className="w-3 h-3 text-green-500 group-hover:text-green-600 transition-colors duration-200" />
                                    )}
                                  </Collapsible.Trigger>
                                      ...
                                      <div className="pl-2 space-y-1 mt-1">
                                        {children.map((child, index) => (
                                          ...
                                          >
                                            ...
                                            <span className="block truncate group-hover/leaf:font-medium transition-all duration-200">
                                              {child.value}
                                            </span>
                                          </button>
                                        ))}
                    ...
                    >
                      <Collapsible.Trigger className="w-full text-left py-3 px-4 rounded-xl hover:bg-gradient-to-r hover:from-gray-50/80 hover:to-blue-50/80 transition-all duration-300 ease-out flex items-center justify-between group-hover:shadow-sm border border-transparent hover:border-gray-100">
                        ...
                        {healthcareCategoriesData.length > 0 ? (
                          <ChevronDownIcon
                            className={`w-4 h-4 text-gray-400 group-hover:text-gray-600 transform transition-all duration-300 ease-out ${
                              openSections.healthcare ? "rotate-180" : ""
                            }`}
                          />
                        ) : (
                          <FaFileExcel className="w-4 h-4 text-green-500 group-hover:text-green-600 transition-colors duration-200" />
                      ...
                      <Collapsible.Content className="overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-top-1 data-[state=open]:slide-in-from-top-1">
                        <div className="px-4 pb-2 space-y-1">
                          {healthcareCategoriesData.map(
                            ({ key: hcKey, value: hcValue }) => {
                              const children =
                                hcKey === "healthcare"
                                  ? getHealthcareChildren()
                                  : getSocialSecurityChildren();
                              const isHealthcareSubOpen =
                                openSections[`healthcare-${hcKey}`] || false;
